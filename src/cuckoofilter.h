...
public:
 ...
 public:
  ...
  size_t TotalSlots() const { return table_->SizeInTags(); }

  // number of current inserted items;
  //size_t Size() const { return num_items_; }
  size_t Size() const { return table_->TotalItems(); }
...
Status CuckooFilter<ItemType, bits_per_item, TableType>::AddImpl(
  ...
  for (uint32_t count = 0; count < kMaxCuckooCount; count++) {
    ...
    oldtag = 0;
    ReturnCode res = table_->ConInsertTagToBucket(curindex, curtag, kickout, oldtag);
    if (res == ReturnCode::Succ) {
      //num_items_++;
      //__sync_fetch_and_add(&num_items_, 1);
      return Ok;
    }
    if (res == ReturnCode::Exists) {
      return Retry;
    }
...
Status CuckooFilter<ItemType, bits_per_item, TableType>::Contain(
  ...
  if (found || table_->ConFindTagInBuckets(i1, i2, tag)) {
    return Ok;
  } else {
    return NotFound;
  }
...
Status CuckooFilter<ItemType, bits_per_item, TableType>::Delete(
  ...
  if (table_->ConDeleteTagFromBucket(i1, tag)) {
    num_items_--;
    goto TryEliminateVictim;
  } else if (table_->ConDeleteTagFromBucket(i2, tag)) {
    num_items_--;
    goto TryEliminateVictim;
